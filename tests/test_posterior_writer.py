...
def test_is_hdf5_output_path_matches_supported_extensions(tmp_path):
    assert writer._is_hdf5_output_path("posteriors.h5")
    assert writer._is_hdf5_output_path("posteriors.hdf5")
    assert writer._is_hdf5_output_path("posteriors.H5")
    assert writer._is_hdf5_output_path("posteriors.HDF5")
    assert writer._is_hdf5_output_path(tmp_path / "posteriors.h5")
...
def test_resolve_writer_format_auto_detects_hdf5_extensions():
    ...
    assert writer._resolve_writer_format("posteriors.h5", None) == "hdf5"
